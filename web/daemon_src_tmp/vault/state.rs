...
pub struct DaemonState {
    ...
    // Key is uid only — lockout persists across WebSocket reconnections so an
    // attacker cannot bypass the exponential back-off by opening a new connection.
    lockout_map: Mutex<HashMap<u32, (u32, Instant)>>,
...
impl DaemonState {
    ...
    fn check_unlock_lockout(&self, uid: u32, _conn_id: u64) -> Result<(), VaultError> {
        ...
        if let Some((_, expiry)) = map.get(&uid) {
            if *expiry > now { return Err(VaultError::Auth("locked out".into())); }
        }
        Ok(())
    }
    ...
    pub fn record_failed_unlock(&self, uid: u32, _conn_id: u64) {
        ...
        let secs = LOCKOUT_SCHEDULE_SECS[count.min(LOCKOUT_SCHEDULE_SECS.len() - 1)];
        if secs > 0 {
    ...
    pub fn reset_unlock_counter(&self, uid: u32, _conn_id: u64) {
        self.lockout_map.lock().unwrap().remove(&uid);
    ...
    fn verify_master_password_inner(&self, password: &[u8]) -> Result<(), VaultError> {
        ...
        let header = self.read_header()?;
        let salt_bytes = hex::decode(&header.argon2_salt).map_err(|_| VaultError::Crypto("invalid salt".into()))?;
        let mut salt = [0u8; 32];
        ...
        let mut vmk_plain = match nonce_vec.len() {
            ...
            _ => return Err(VaultError::Crypto("invalid vmk_nonce length".into())),
        };
        ...
        // M-02 fix: zeroize vmk_plain before drop — it is a heap-allocated Vec.
        vmk_plain.zeroize();
        result
    }
    ...
    pub fn update_login_policy(&self, p: bool, t: bool, e: bool) -> Result<(), VaultError> {
        let _guard = self.db.lock().unwrap();
    ...
    pub fn lock(&self) {
        ...
        self.lockout_map.lock().unwrap().clear();
    }
