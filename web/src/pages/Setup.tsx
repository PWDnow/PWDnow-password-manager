...
import { apiFetch, hasServerSession as _hasServerSession } from '../utils/api';
...
function UbuntuProModal({ onClose, onActivated }: { onClose: () => void; onActivated: () => void }) {
  ...
  return (
    <ModalShell accentColor="#E95420" icon={Shield} title={t('setup.ubuntuProTitle', 'Ubuntu Pro')} subtitle={t('setup.ubuntuProSubtitle', 'Free for personal use · up to 5 machines')} onClose={onClose}>
      {/* Benefits */}
      <div className="space-y-3">
        <p className="text-sm text-on-surface-variant leading-relaxed">
          {t('setup.ubuntuProDesc', 'Ubuntu Pro expands your Ubuntu LTS with enterprise-grade security features at no cost for personal use. Attaching your token will also automatically enable FIPS 140-2 certified modules.')}
        </p>
        <div className="grid grid-cols-1 gap-2">
          {[
            { label: t('setup.proFeature1', 'Expanded Security Maintenance (ESM)'),  sub: t('setup.proFeature1Sub', '10 years of security patches for 30,000+ packages') },
            { label: t('setup.proFeature2', 'FIPS 140-2 certified modules'),         sub: t('setup.proFeature2Sub', 'Certified OpenSSL, kernel crypto — enabled automatically on attach') },
            { label: t('setup.proFeature3', 'Kernel Livepatch'),                     sub: t('setup.proFeature3Sub', 'Apply kernel security patches without rebooting') },
            { label: t('setup.proFeature4', 'USG / CIS hardening'),                  sub: t('setup.proFeature4Sub', 'Automated compliance with DISA-STIG & CIS benchmarks') },
          ].map(({ label, sub }) => (
      ...
      <div>
        <label htmlFor="pro-token" className="block text-sm font-bold text-black dark:text-white mb-2">
          {t('setup.proHaveLicence', 'I have a licence')}
        </label>
        <input
          id="pro-token"
          type="text"
          ...
          placeholder={t('setup.proTokenPlaceholder', 'Paste your Ubuntu Pro token…')}
      ...
      {command && (
        <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
          <p className="text-xs font-bold text-black dark:text-white">
            {t('setup.runCommand', 'Run this command in your terminal:')}
          </p>
          ...
          <p className="text-[11px] text-on-surface-variant">
            {t('setup.rebootNote', 'A reboot may be required after FIPS modules are installed.')}
          </p>
        </motion.div>
      )}
      ...
      >
        {t('setup.doneRunCommand', "Done - I've run the command")}
      </button>
    </ModalShell>
  );
}
...
function FipsEnableModal({ onClose, onEnabled }: { onClose: () => void; onEnabled: () => void }) {
  ...
  return (
    <ModalShell accentColor="#2563eb" icon={Lock} title={t('setup.fipsTitle', 'Enable FIPS 140-2')} subtitle={t('setup.fipsSubtitle', 'Requires Ubuntu Pro · reboot needed after')} onClose={onClose}>
      <div className="space-y-4">
        <p className="text-sm text-on-surface-variant leading-relaxed">
          {t('setup.fipsDesc', 'This will install FIPS 140-2 certified cryptographic modules (OpenSSL, kernel, libgcrypt) and replace the standard Ubuntu packages.')}
        </p>
        <p className="text-xs text-on-surface-variant/70 italic leading-relaxed">
          {t('setup.fipsNote', "Ubuntu Pro currently ships FIPS 140-2 validated modules; FIPS 140-3 module rollout is in progress upstream. PWDnow's vault daemon cryptography is FIPS 140-3-aligned — see Technical Reference §6.0.")}
        </p>
        <div className="space-y-2">
          {[
            t('setup.fipsItem1', 'FIPS 140-2 validated OpenSSL & libssl'),
            t('setup.fipsItem2', 'FIPS 140-2 kernel cryptographic API'),
            t('setup.fipsItem3', 'Required for NIST / CMMC / FedRAMP compliance'),
          ].map(line => (
        ...
        <div className="flex items-start gap-3 p-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800">
          ...
          <p className="text-[11px] text-amber-800 dark:text-amber-300 font-medium">
            {t('setup.fipsRebootWarning', 'A reboot will be required after FIPS modules are installed. The system will not use FIPS until restarted.')}
          </p>
        </div>
        ...
        <div className="flex gap-3 pt-1">
          ...
          <button onClick={onEnabled}
            ...
            {t('setup.doneRunIt', "Done - I've run it")}
          </button>
        </div>
      </div>
    </ModalShell>
        ...
        <div className="bg-white dark:bg-surface-container-low rounded-3xl shadow-2xl border border-outline-variant/10 overflow-hidden">
          <AnimatePresence mode="wait" custom={direction}>
            ...
            {step === 0 && (
              ...
              >
                ...
                <div className="flex flex-col items-center text-center gap-5">
                  ...
                  <div>
                    <h1 className="font-headline font-black text-3xl md:text-4xl tracking-tight text-black dark:text-white mb-3">
                      {t('setup.welcomeTitle', 'Welcome to PWDnow')}
                    </h1>
                    <p className="text-on-surface-variant text-base leading-relaxed max-w-md">
                      {t('setup.welcomeDesc', 'Your self-hosted, open-source password manager — built for security-conscious teams and individuals.')}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {[
                    { icon: Lock,   label: t('setup.featureEncrypted', 'End-to-end encrypted'),  sub: t('setup.featureEncryptedSub', 'AES-256 + Argon2id') },
                    { icon: Globe,  label: t('setup.featureOpenSource', 'Open source'),           sub: t('setup.featureOpenSourceSub', 'MIT licensed') },
                    { icon: Shield, label: t('setup.featureFips', 'FIPS-ready'),                  sub: t('setup.featureFipsSub', 'FIPS 140-2 support') },
                ...
                <div className="p-5 rounded-2xl bg-surface-container-low border border-outline-variant/10 space-y-3 text-sm text-on-surface-variant leading-relaxed">
                  <p>
                    {t('setup.aboutText1', 'PWDnow stores all credentials locally — no cloud, no telemetry, no vendor lock-in. You own your data.')}
                  </p>
                  <p>
                    {t('setup.aboutText2', 'Supports TOTP, WebAuthn hardware keys, and email OTP for multi-factor authentication. Full audit log. Recovery kit. Emergency access controls.')}
                  </p>
                ...
                >
                  {t('setup.getStarted', 'Get Started')} <ArrowRight size={18} />
                </button>
              </motion.div>
            )}
            ...
            {step === 1 && (
              ...
              >
                <div>
                  <h2 className="font-headline font-black text-2xl text-black dark:text-white mb-2">{t('setup.sysReqTitle', 'System Requirements')}</h2>
                  <p className="text-on-surface-variant text-sm leading-relaxed">
                    {t('setup.sysReqDesc', 'Scanning your host system for compatibility and security features.')}
                  </p>
                </div>
                ...
                {scanning ? (
                  <div className="flex flex-col items-center justify-center gap-4 py-10">
                    ...
                    <p className="text-sm text-on-surface-variant font-bold">{t('setup.scanning', 'Scanning system…')}</p>
                  </div>
                ) : sysInfo?.error ? (
                  <div className="p-5 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-2xl flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-bold text-sm text-amber-800 dark:text-amber-300 mb-1">{t('setup.scanError', 'Could not scan system')}</p>
                  ...
                  <div className="space-y-3">
                    ...
                    <InfoCard
                      icon={Monitor}
                      label={t('setup.labelOs', 'Operating System')}
                    ...
                    <InfoCard
                      icon={Server}
                      label={t('setup.labelHostname', 'Hostname')}
                    ...
                    <InfoCard
                      icon={Cpu}
                      label={t('setup.labelKernel', 'Kernel / Architecture')}
                    ...
                    {sysInfo.os === 'ubuntu' && (
                      <InfoCard
                        icon={Shield}
                        label={t('setup.labelUbuntuPro', 'Ubuntu Pro')}
                        value={sysInfo.ubuntu_pro ? t('setup.ubuntuProActive', 'Active — expanded security patches') : t('setup.ubuntuProNotActive', 'Not active')}
                        status={sysInfo.ubuntu_pro ? 'ok' : 'neutral'}
                        action={!sysInfo.ubuntu_pro ? { label: t('setup.enable', 'Enable'), onClick: () => setShowProModal(true) } : undefined}
                      />
                    )}
                    ...
                    {(sysInfo.os === 'ubuntu' || sysInfo.fips_enabled) && (
                      <InfoCard
                        icon={Lock}
                        label={t('setup.labelFips', 'FIPS 140-2 Kernel')}
                        value={sysInfo.fips_enabled ? t('setup.fipsEnabled', 'Enabled — approved crypto modules active') : t('setup.fipsNotEnabled', 'Not enabled')}
                        status={sysInfo.fips_enabled ? 'ok' : 'neutral'}
                        action={
                          !sysInfo.fips_enabled && sysInfo.ubuntu_pro
                            ? { label: t('setup.enable', 'Enable'), onClick: () => setShowFipsModal(true) }
                            : undefined
                        }
                      />
                    )}
                    ...
                    <InfoCard
                      icon={Fingerprint}
                      label={t('setup.labelTpm', 'TPM')}
                      value={
                        sysInfo.tpm.present
                          ? `TPM ${sysInfo.tpm.version} · ${tpmTypeLabel(sysInfo.tpm.type)}`
                          : t('setup.notDetected', 'Not detected')
                      }
                    ...
                    <InfoCard
                      icon={Key}
                      label={t('setup.labelHsm', 'Hardware Security Module (HSM)')}
                      value={sysInfo.hsm.present ? sysInfo.hsm.type || t('setup.detected', 'Detected') : t('setup.notDetected', 'Not detected')}
                      status={sysInfo.hsm.present ? 'ok' : 'neutral'}
                    />
                    {/* Drive encryption */}
                    <InfoCard
                      icon={HardDrive}
                      label={t('setup.labelDriveEncryption', 'Drive Encryption')}
                      value={
                    ...
                    {sysInfo.drive_encryption.csfc && (
                      <div className="flex items-center gap-3 p-4 rounded-xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800">
                        ...
                        <p className="text-xs font-bold text-blue-800 dark:text-blue-300">
                          {t('setup.csfcBanner', 'CSfC dual-layer encryption detected — two independent encryption layers are active.')}
                        </p>
                      </div>
                    )}
                    ...
                    {sysInfo.drive_encryption.fips_140 === 'true' && (
                      <div className="flex items-center gap-3 p-4 rounded-xl bg-green-50 dark:bg-green-950/40 border border-green-200 dark:border-green-800">
                        ...
                        <p className="text-xs font-bold text-green-800 dark:text-green-300">
                          {t('setup.fipsDriveBanner', 'FIPS 140 drive encryption — AES-XTS-512 with FIPS-validated kernel modules.')}
                        </p>
                      </div>
                    )}
                    ...
                    {sysInfo.fips_enabled && sysInfo.drive_encryption.fips_140 !== 'true' && (
                      <div className="flex items-center gap-3 p-4 rounded-xl bg-green-50 dark:bg-green-950/40 border border-green-200 dark:border-green-800">
                        <CheckCircle size={18} className="text-green-600 shrink-0" />
                        <p className="text-xs font-bold text-green-800 dark:text-green-300">
                          {t('setup.fipsKernelBanner', 'FIPS 140-2 compliant — all kernel cryptographic operations use approved modules.')}
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="flex gap-3">
                  ...
                  >
                    {t('setup.continue', 'Continue')} <ArrowRight size={18} />
                  </button>
                </div>
              </motion.div>
            )}
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-on-surface-variant text-sm leading-relaxed max-w-sm">
                    {t('setup.completeDesc', 'PWDnow is ready. Create your admin account to start managing credentials securely.')}
                  </p>
                </div>
                ...
                <div className="w-full space-y-2 text-left">
                  {[
                    { icon: Lock,        text: t('setup.summaryEncryption', 'End-to-end encryption enabled') },
                    { icon: ShieldCheck, text: t('setup.summaryDaemon', 'Vault daemon ready') },
                    { icon: Monitor,     text: sysInfo ? t('setup.summaryOs', 'Running on {{os}}', { os: sysInfo.os_pretty }) : t('setup.summaryScanned', 'System scanned') },
                  ].map(({ icon: Icon, text }) => (
                ...
                <div className="w-full flex gap-3">
                  ...
                  >
                    ...
                    {completing ? t('setup.starting', 'Starting…') : t('setup.createAdmin', 'Create Admin Account')}
                  </button>
                </div>
              </motion.div>
            )}
