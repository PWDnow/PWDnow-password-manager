              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl ${variant === 'danger' ? 'bg-red-600/10' : 'bg-blue-600/10'} flex items-center justify-center`}>
                    <AlertTriangle size={20} className={variant === 'danger' ? 'text-red-600' : 'text-blue-600'} aria-hidden="true" />
                  </div>
                  <h2 id="confirm-modal-title" className="text-xl font-bold text-slate-900 dark:text-white">{title}</h2>
                </div>
                <button onClick={onClose} aria-label={t('common.close', 'Close')} className="p-2 hover:bg-slate-100 dark:hover:bg-white/5 rounded-lg transition-colors">
                  <X size={20} className="text-slate-500" aria-hidden="true" />
                </button>
              </div>

              <p id="confirm-modal-desc" className="text-slate-600 dark:text-slate-300 text-sm mb-8 leading-relaxed">
                {message}
              </p>
