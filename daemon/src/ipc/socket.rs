...
impl SocketListener {
    ...
    pub async fn run(self) -> Result<(), VaultError> {
        ...
        // H-4 / M-2 fix: skip the auto-lock when any request is in-flight. The
        // existing in_flight_requests counter (already incremented in
        // handle_connection before dispatch) is exactly the right signal —
        // it covers the slow-Argon2id-unlock window and any other long-running
        // handler. Without this guard, the idle ticker could lock the vault
        // mid-handler and a subsequent touch() would race against the zeroed VMK,
        // causing crypto calls to operate on the wrong key.
        let state_idle = Arc::clone(&state);
        tokio::spawn(async move {
            let mut ticker = tokio::time::interval(Duration::from_secs(60));
            loop {
                ticker.tick().await;
                if state_idle.is_locked() { continue; }
                if state_idle.in_flight_requests.load(Ordering::Acquire) > 0 { continue; }
                if state_idle.idle_secs() < IDLE_TIMEOUT_SECS { continue; }
                // Re-verify the in-flight counter immediately before the destructive
                // call, since a new request may have arrived in the microseconds
        ...
        // M-3 fix: periodically prune expired lockout-map entries. Without this,
        // an attacker who hits the unlock failure threshold once and never
        // reconnects leaves a permanent entry. check_unlock_lockout already
        // prunes on every call, but only for accounts that retry — idle attackers
        // accumulate forever. Pattern mirrors the challenge prune at
        // state.rs:145-146 (in-line on every challenge allocation).
        let state_prune = Arc::clone(&self.state);
        tokio::spawn(async move {
            let mut ticker = tokio::time::interval(Duration::from_secs(300)); // 5 min
            loop {
                ticker.tick().await;
                state_prune.prune_lockout_map();
            }
        });
...
async fn dispatch(state: &DaemonState, uid: u32, conn_id: u64, request: Request) -> Response {
    match request {
        ...
        Request::UnlockWithRecoveryKey { recovery_key } => {
            let res = state.unlock_with_recovery_key(&recovery_key, uid, conn_id);
            match res {
                Err(e) => {
                ...
                Ok(sess) => {
                    let _ = state.audit_log("UNLOCK_RECOVERY", Some(&sess.user_id));
                    let ticket = state.wipe_ticket_bytes().unwrap_or_default();
                    let vmk_guard = state.vmk_read_guard();
                    let (ct, nonce) = if let Some(vmk) = vmk_guard.as_ref() {
                        let mut vmk_bytes = [0u8; 32];
        ...
        Request::ForensicWipe { wipe_ticket_ciphertext, wipe_ticket_nonce } => {
            // D-1 fix: decrypt the wipe ticket UNDER the vmk read guard, then
            // drop the guard BEFORE calling forensic_wipe. forensic_wipe → self.lock()
            // takes vmk.write(); holding vmk.read() across that path deadlocks the
            // dispatch task forever (the wipe silently never fires, vault.db stays
            // on disk, and only the 120 s deadline returns a 504 to the client).
            // Splitting the closure ensures the read guard drops before the
            // destructive phase.
            let ticket_res: Result<Vec<u8>, VaultError> = state.with_vmk(|vmk|
                state.decrypt_wipe_ticket(&wipe_ticket_ciphertext, &wipe_ticket_nonce, vmk));
            let res = ticket_res.and_then(|ticket| state.forensic_wipe(&ticket));
            match res {
        ...
        Request::EnrollRecoveryKey { session_token, recovery_key } => {
            auth_then!(state, uid, session_token, {
                match state.enroll_recovery_key(&recovery_key) {
                    Ok(_) => Response::Ok,
                    Err(e) => err(500, e),
                }
            })
        ...
        Request::UpdateLoginPolicy { session_token, password_login_enabled, totp_enabled, email_otp_enabled, duress_max_attempts } => {
            auth_then!(state, uid, session_token, {
                match state.update_login_policy(password_login_enabled, totp_enabled, email_otp_enabled, duress_max_attempts) {
                    Ok(_) => Response::Ok,
                    Err(e) => err(500, e),
                }
            })
